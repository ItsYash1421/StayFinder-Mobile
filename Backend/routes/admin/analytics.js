...
// Root analytics endpoint for mobile app
router.get('/', async (req, res) => {
  try {
    ...
    listings.forEach(listing => {
      ...
      // Calculate average rating and price for this location
      const locationListings = listings.filter(l => (l.location || 'Unknown') === loc);
      locationMap[loc].avgRating = locationListings.reduce((sum, l) => sum + (l.rating || 0), 0) / locationListings.length;
      locationMap[loc].avgPrice = locationListings.reduce((sum, l) => sum + (l.price || 0), 0) / locationListings.length;
    });
    
    // Then add booking data to locations
    confirmedBookings.forEach(b => {
      const listing = listings.find(l => l._id.toString() === (b.listingId?.toString() || b.propertyId?.toString()));
...
// Simple endpoint for mobile app admin dashboard
router.get('/mobile-stats', async (req, res) => {
  try {
    ...
    const confirmedBookings = await Booking.find({ status: { $in: ['confirmed', 'approved'] } });
    ...
    console.error("Error fetching mobile admin stats:", error);
