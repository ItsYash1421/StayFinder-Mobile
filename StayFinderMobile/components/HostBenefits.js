...
const BENEFITS = [
  {
    ...
    description: 'Our smart pricing tool adjusts rates based on demand, seasonality, and local events',
    stat: '30% higher average income',
  },
  {
    icon: (color, size) => <Feather name="shield" size={size} color={color} />,
    title: 'Full Protection',
    description: '$1M property damage protection and liability coverage included at no extra cost',
  ...
  },
  {
    ...
    description: 'Verified ID, payment screening, and review system ensure respectful guests',
    stat: '4.9/5 host satisfaction',
  },
  {
    icon: (color, size) => <AntDesign name="Safety" size={size} color={color} />,
    title: 'Superhost Program',
    description: 'Earn badges, better placement, and exclusive perks as you build your reputation',
    stat: '20% more bookings',
  },
  {
    icon: (color, size) => <Feather name="clock" size={size} color={color} />,
    title: 'Flexible Control',
    description: 'Set your own availability, minimum stays, and cancellation policies',
    stat: '100% calendar control',
  },
  {
    icon: (color, size) => <Feather name="dollar-sign" size={size} color={color} />,
    title: 'Fast Payouts',
    description: 'Get paid via direct deposit, PayPal, or wire transfer within 24 hours of check-in',
    stat: 'No hidden fees',
  },
  {
    icon: (color, size) => <Feather name="globe" size={size} color={color} />,
    title: 'Global Reach',
    description: 'Connect with travelers from 120+ countries with built-in translation for 20 languages',
    stat: 'International exposure',
  },
];

const PERKS = [
  { icon: <Feather name="home" size={20} color={COLORS.primary} />, text: 'Professional photography service' },
  { icon: <Feather name="message-square" size={20} color={COLORS.primary} />, text: 'Priority customer support' },
  { icon: <Feather name="credit-card" size={20} color={COLORS.primary} />, text: 'Exclusive discount on cleaning services' },
];
...
export default function HostBenefits() {
  ...
  return (
    <View style={styles.section}>
      <Text style={styles.title}><Text style={{ color: COLORS.primary }}>Host</Text> With Confidence</Text>
      <Text style={styles.subtitle}>Join 500,000+ hosts earning an average of $15,000/year on our platform</Text>
      
      <Image
        source={{ uri: 'https://images.unsplash.com/photo-1600585154340-be6161a56a0c?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=600&q=80' }}
