...
export default function GuestRequestScreen({ navigation }) {
  ...
  const handleAction = async (bookingId, action) => {
    try {
      const response = await api.post(`/api/user/${action}-booking`, {
        bookingId,
      }, {
        headers: { Authorization: `Bearer ${token}` },
      });
      ...
      if (response.data.success) {
        ...
        toast.showToast(`Booking ${actionText} successfully!`, "success");
        // Refresh the requests
        fetchRequests();
      } else {
        const errorMessage = response.data.message || `Failed to ${action} booking`;
